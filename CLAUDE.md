...
- **`solutionResultKind` taxonomy**: classifies solver output into `"Rules"` (fully determined list of rules, association with `Residual -> True`, or residuals involving only transition flows `j[_,_,_]`), `"NoSolution"` (empty list or `Residual -> False`), `"Branched"` (residual `Or` involving primary variables), `"Underdetermined"` (primary edge flows `j[_,_]` or values `u[__]` remain free but no primary-variable disjunction — their values do not affect the rest of the solution), `"ResidualLogic"` (other symbolic residual), `"TIMEOUT"`, or `"Unknown"`. Use `solutionVariableDiagnostics[sys, sol]` when a system is available to report transition-flow determinacy separately.
