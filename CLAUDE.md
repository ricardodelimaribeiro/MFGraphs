...
- **`solutionResultKind` taxonomy**: classifies solver output into `"Rules"` (fully determined list of rules, association with `Equations -> True`, or residuals involving only transition flows `j[_,_,_]`), `"NoSolution"` (empty list or `Equations -> False`), `"Branched"` (residual `Or` involving primary variables), `"Parametric"` (primary edge flows `j[_,_]` or values `u[__]` remain free but no primary-variable disjunction), `"Residual"` (other symbolic residual), `"TIMEOUT"`, or `"Unknown"`. Use `solutionVariableDiagnostics[sys, sol]` when a system is available to report transition-flow determinacy separately.
