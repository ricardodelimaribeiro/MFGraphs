...
DataToEquations[Data_?AssociationQ] := Module[{BG, EntranceVertices, InwardVertices, InEdges, ExitVertices, 
 ...
 OtherWay[{c_, a_ \[DirectedEdge] b_}] := {If[c === a, b, a], 
   a \[DirectedEdge] 
    b} (*example:OtherWay[{a,a\[DirectedEdge]b}]={b,a\[DirectedEdge]b}\
*);
...
conecting the first two and the last two.If this is not feasible,
  error message with {a,b,c}!*)(EL = EdgeList[FG];
...
EqValueAuxiliaryEdges = 
 ...
 Association[{"BG" -> BG, "EntranceVertices" -> EntranceVertices, 
   ...
   "VL" -> VL, "EL" -> EL, "BEL" -> BEL, "jargs" -> jargs, "js" -> js,
    "jvars" -> jvars, "FVL" -> FVL, 
