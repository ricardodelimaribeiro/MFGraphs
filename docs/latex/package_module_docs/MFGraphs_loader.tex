...
\subsection*{Optional subpackages}

Two files in the package directory are intentionally \emph{not} loaded by \texttt{Needs["MFGraphs`"]}: \texttt{Tawaf.wl} (unrolled circumambulation scenario builder) and \texttt{numericOracle.wl} (LP-relaxation oracle, the only file in the package that introduces floating-point work). They are loaded explicitly after the main package:
...
The high-level wrapper for the oracle path is \texttt{solveScenarioWithOracle}, which composes \texttt{addSymmetryEqualities} $\to$ \texttt{numericOracleClassify} $\to$ \texttt{addOracleEqualities} $\to$ \texttt{activeSetReduceSystem} and falls back to the unpruned solve when the oracle over-prunes.
...
This call declares the top-level \texttt{MFGraphs`} context and specifies the active dependency contexts that should be loaded with it: \texttt{primitives`}, \texttt{utilities`}, \texttt{scenarioTools`}, \texttt{examples`}, \texttt{unknownsTools`}, \texttt{systemTools`}, \texttt{solversTools`}, \texttt{orchestrationTools`}, and \texttt{graphicsTools`}. The optional \texttt{Tawaf`} and \texttt{numericOracle`} contexts are intentionally absent from this list.
