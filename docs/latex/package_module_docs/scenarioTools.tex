...
\section{Scenario schema}

A completed \texttt{scenario[\textellipsis]} wraps an association with the following top-level keys.

\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{longtable}{@{}p{2.5cm} p{12.0cm}@{}}
\toprule
\textbf{Key} & \textbf{Contents} \\
\midrule
\endhead
\texttt{"Model"} & The user-supplied network description: \texttt{"Vertices"}, \texttt{"Adjacency"} (or \texttt{"Graph"}), \texttt{"Entries"} (vertex/mass pairs), \texttt{"Exits"} (vertex/terminal-cost pairs), \texttt{"Switching"} (cost rows or association keyed by transition triples). \\
\texttt{"Hamiltonian"} & Normalized Hamiltonian parameters: \texttt{"Alpha"}, \texttt{"V"}, \texttt{"G"}, plus the edge-keyed override maps \texttt{"EdgeAlpha"}, \texttt{"EdgeV"}, \texttt{"EdgeG"}. \\
\texttt{"Topology"} & Cached auxiliary-graph data produced by \texttt{buildAuxiliaryTopology}: the undirected base graph, auxiliary entry/exit vertices, augmented graph, directed auxiliary pairs, and admissible transition triples. \\
\texttt{"Identity"} & Optional identifying metadata (name, source). \\
\texttt{"Benchmark"} & Optional benchmark-related fields (filled with defaults by \texttt{completeScenario}). \\
\bottomrule
\end{longtable}
\end{center}

The \texttt{"Topology"} block is computed once inside \texttt{makeScenario} and is the reason every downstream stage runs in time proportional to its own work rather than re-walking the graph.
...
\texttt{buildAuxTriples[auxGraph]} returns admissible transition triples $\{v_{\text{in}}, v_{\text{mid}}, v_{\text{out}}\}$ derived from an auxiliary graph. These triples are the basis of transition-flow variables and switching-cost logic.
...
\subsection*{\texttt{disjointBoundaryVerticesQ}}

\texttt{disjointBoundaryVerticesQ[model]} returns \texttt{True} when the entry-vertex set and the exit-vertex set do not overlap. The active scenario kernel rejects models where a single vertex is simultaneously declared an entry and an exit, since the auxiliary-topology construction would then attach two incompatible boundary fixtures to the same node.

\subsection*{\texttt{boundaryVerticesPresentQ}}

\texttt{boundaryVerticesPresentQ[model]} returns \texttt{True} when every entry and exit vertex named in the model also appears in the \texttt{"Vertices"} list. It catches the common authoring mistake of declaring boundary data on a vertex that was never added to the graph.
