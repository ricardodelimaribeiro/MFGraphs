...
\texttt{gridScenario[dims, entries, exits, sc, alpha, V, g]} constructs a scenario from a directed \texttt{GridGraph}. When \texttt{dims} is \texttt{\{n\}}, it creates a chain on vertices \texttt{1..n}; when \texttt{dims} is \texttt{\{r, c\}}, it creates an $r \times c$ grid with row-major integer labels.
...
\texttt{cycleScenario[n, entries, exits, sc, alpha, V, g]} constructs a directed cycle $1 \to 2 \to \cdots \to n \to 1$. It is the simplest way to obtain cyclic benchmark scenarios without spelling out adjacency matrices manually.
...
\subsection*{\texttt{getExampleScenarioMetadata}}

\texttt{getExampleScenarioMetadata[key]} returns provenance and source metadata for a built-in example, when available. It returns \texttt{\$Failed} for keys that are not in the registry. Use it when a script or paper-table builder needs to attach a human-readable origin to a scenario alongside its solution.

\subsection*{\texttt{listExampleScenarios}}

\texttt{listExampleScenarios[]} returns the sorted list of all keys recognised by \texttt{getExampleScenario} --- both the numeric legacy keys (\texttt{3}, \texttt{11}--\texttt{18}, \texttt{20}--\texttt{23}, \texttt{27}, \texttt{104}, \texttt{105}) retained for backward compatibility and the newer named keys (\texttt{"Jamaratv9"}, \texttt{"Grid0303"}, \texttt{"Camilli 2015 simple"}, \textellipsis). It is the canonical discovery API: tests, benchmarks, and example-coverage smoke runs iterate over its return value rather than hard-coding key lists.
